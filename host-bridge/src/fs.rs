...
impl FsBridge {
    ...
    async fn read(&self, params: Value) -> Result<Value> {
        ...
        let content = match request.encoding.to_lowercase().as_str() {
            "utf-8" | "utf8" => match String::from_utf8(bytes) {
                Ok(s) => s,
                Err(e) => {
                    return Ok(json!({
                        "ok": false,
            ...
            "ascii" => match String::from_utf8(bytes.clone()) {
    ...
    async fn write(&self, params: Value) -> Result<Value> {
        ...
        let bytes = match request.encoding.to_lowercase().as_str() {
            ...
            "base64" => match BASE64.decode(&request.content) {
                Ok(b) => b,
                Err(e) => {
                    return Ok(json!({
                        "ok": false,
...
mod tests {
    ...
    async fn test_file_size_limit() {
        ...
        let temp_dir = TempDir::new().unwrap();
        let bridge_with_sandbox =
            FsBridge::with_sandbox(temp_dir.path().to_path_buf()).with_max_file_size(100);
